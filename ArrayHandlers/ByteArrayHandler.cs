...
namespace VisionNET
{
    ...
    /// </summary>
    [Serializable]
    public sealed class ByteArrayHandler : IArrayHandler<byte>
    {
        private byte[,,] _data;
        private int _rows;
        ...
        public unsafe byte[,] ExtractChannel(int channel)
        {
            byte[,] data = new byte[_rows, _columns];
            fixed (byte* src = _data, dst = data)
            {
                byte* srcPtr = src;
                byte* dstPtr = dst;
                srcPtr += channel;
        ...
        public unsafe byte[, ,] ExtractRectangle(int startRow, int startColumn, int rows, int columns)
        {
            byte[, ,] patch = new byte[rows, columns, _channels];
            int stride = _columns * _channels;
            fixed (byte* src = _data, dst = patch)
            {
                byte* tl = src;
                byte* bl = src + (_rows - 1) * stride;
                byte* srcPtr = src + startRow * stride + startColumn * _channels;
                byte* dstPtr = dst;
        ...
        {
            _data = new byte[rows, columns, channels];
            _rows = rows;
            _columns = columns;
            _channels = channels;
        }
        ...
        public void Clear()
        {
            _data = new byte[_rows, _columns, _channels];
        }
