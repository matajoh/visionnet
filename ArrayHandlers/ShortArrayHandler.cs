...
namespace VisionNET
{
    ...
    /// </summary>
    [Serializable]
    public sealed class ShortArrayHandler : IArrayHandler<short>
    {
        private short[,,] _data;
        private int _rows;
        ...
        public ShortArrayHandler(int rows, int columns, int channels)
        {
        ...
        public unsafe short ComputeRectangleSum(int startRow, int startColumn, int rows, int columns, int channel)
        {
            ...
            if (_isIntegral)
            {
                return (short)(_data[minRow, minColumn, channel] +
            ...
            else
            {
                ...
                short sum = 0;
                fixed (short* src = _data)
                {
                    short* ptr = src + minRow * stride + minColumn * _channels + channel;
        ...
        public unsafe short[,] ExtractChannel(int channel)
        {
            short[,] data = new short[_rows, _columns];
            fixed (short* src = _data, dst = data)
            {
                short* srcPtr = src;
                short* dstPtr = dst;
                srcPtr += channel;
        ...
        public unsafe short[, ,] ExtractRectangle(int startRow, int startColumn, int rows, int columns)
        {
            short[, ,] patch = new short[rows, columns, _channels];
            int stride = _columns * _channels;
            fixed (short* src = _data, dst = patch)
            {
                short* tl = src;
                short* bl = src + (_rows - 1) * stride;
                short* srcPtr = src + startRow * stride + startColumn * _channels;
                short* dstPtr = dst;
        ...
        {
            _data = new short[rows, columns, channels];
            _rows = rows;
            _columns = columns;
            _channels = channels;
        }
        ...
        public void Clear()
        {
            _data = new short[_rows, _columns, _channels];
        }
    }
}
