...
namespace VisionNET
{
    ...
    /// </summary>
    [Serializable]
    public sealed class FloatArrayHandler : IArrayHandler<float>
    {
        ...
        public FloatArrayHandler(int rows, int columns, int channels)
        {
        ...
        public FloatArrayHandler(float[, ,] data, bool isIntegral)
        {
            SetData(data);
        }
        ...
        public unsafe float ComputeRectangleSum(int startRow, int startColumn, int rows, int columns, int channel)
        {
            ...
            if (_isIntegral)
            {
                return _data[minRow, minColumn, channel] +
            ...
            else
            {
                ...
                float sum = 0;
                fixed (float* src = _data)
                {
                    float* ptr = src + minRow * stride + minColumn * _channels + channel;
        ...
        public unsafe float[,] ExtractChannel(int channel)
        {
            float[,] data = new float[_rows, _columns];
            fixed (float* src = _data, dst = data)
            {
                float* srcPtr = src;
                float* dstPtr = dst;
                srcPtr += channel;
        ...
        public unsafe float[, ,] ExtractRectangle(int startRow, int startColumn, int rows, int columns)
        {
            float[, ,] patch = new float[rows, columns, _channels];
            int stride = _columns * _channels;
            fixed (float* src = _data, dst = patch)
            {
                float* tl = src;
                float* bl = src + (_rows - 1) * stride;
                float* srcPtr = src + startRow * stride + startColumn * _channels;
                float* dstPtr = dst;
        ...
        {
            _data = new float[rows, columns, channels];
            _rows = rows;
            _columns = columns;
            _channels = channels;
        }
        ...
        public void Clear()
        {
            _data = new float[_rows, _columns, _channels];
        }
    }
}
